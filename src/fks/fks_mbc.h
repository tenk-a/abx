...
typedef struct Fks_MbcEnv {
	...
	size_t   (*adjustSize)(const char* str, size_t size);
	size_t   (*sizeToChrs)(const char* str, size_t size);
	size_t   (*chrsToSize)(const char* str, size_t chrs);
	int 	 (*cmp)(const char* lp, const char* rp);
} Fks_MbcEnv;
