...
fks_createDirEntries1a(Fks_DirEntries* dirEntries, char const** dirPath, char const* fname
...
fks_foreachDirEntries(Fks_DirEntries* des, Fks_ForeachDirEntCB cb, void* data, Fks_DirEnt_Matchs* mt) FKS_NOEXCEPT
{
    Fks_DirEnt_Matchs   mtBuf;
    if (!mt) {
        memset(&mtBuf, 0, sizeof mtBuf);
    ...
    return fks_foreachDirEntriesMT(des, cb, data, mt);
}

FKS_STATIC_DECL (fks_isize_t)
fks_foreachDirEntriesMT(Fks_DirEntries* des, Fks_ForeachDirEntCB cb, void* data, Fks_DirEnt_Matchs* mt) FKS_NOEXCEPT
{
    fks_isize_t         i, n, cnt;
...
FKS_STATIC_DECL (fks_isize_t) fks_convDirEntPathStatSub(void** ppAry, Fks_DirEntries* des
                                , unsigned int flags, Fks_DirEnt_Matchs* mt) FKS_NOEXCEPT;
...
FKS_LIB_DECL (fks_isize_t)
fks_countDirEntries(size_t* strBytes, Fks_DirEntries* des, Fks_DirEnt_Matchs* mt) FKS_NOEXCEPT
{
    size_t              cnt[2] = {0};
    Fks_DirEnt_Matchs   mtBuf;
    if (!mt) {
    ...
    fks_foreachDirEntries(des, (strBytes) ? fks_countDirEntries_sub2 : fks_countDirEntries_sub, cnt, mt);
    if (strBytes)
        *strBytes = cnt[1];
...
FKS_LIB_DECL (fks_isize_t)
fks_convDirEntPathStats(Fks_DirEntPathStat** ppPathStats , Fks_DirEntries* des, Fks_DirEnt_Matchs* mt) FKS_NOEXCEPT
{
    return fks_convDirEntPathStatSub((void**)ppPathStats, des, FKS_DE_NameStat, mt);
}

FKS_LIB_DECL (fks_isize_t)
fks_convDirEntPaths(char*** pppPaths, Fks_DirEntries* des, Fks_DirEnt_Matchs* mt) FKS_NOEXCEPT
{
    return fks_convDirEntPathStatSub((void**)pppPaths, des, FKS_DE_Tiny, mt);
}
...
fks_createDirEntPathStatSub(void** ppAry, char const* dirPath, unsigned int flags, Fks_DirEnt_Matchs* mt) FKS_NOEXCEPT
{
    ...
    *ppAry = NULL;
    ...
    mt2 = *mt;
    mt2.flags = mt->flags;
    if (fks_createDirEntriesMT(&des, dirPath, &mt2) == NULL)
        return -1;
    if (des.size <= 0)
        return des.size;
    n = fks_convDirEntPathStatSub(ppAry, &des, flags, &mt1);
    fks_releaseDirEntries(&des);
    return n;
}
...
FKS_STATIC_DECL (fks_isize_t)
fks_convDirEntPathStatSub(void** ppAry, Fks_DirEntries* des, unsigned int flags, Fks_DirEnt_Matchs* mt) FKS_NOEXCEPT
{
    fks_isize_t             n;
    fks_isize_t             cnt;
    size_t                  strSz;
    ...
    *ppAry = NULL;
    ...
    flags |= mt->flags;
    mt->flags = flags;
    n = fks_countDirEntries(&strSz, des, mt);
    if (n <= 0)
        return n;
    ...
    cnt = fks_foreachDirEntriesMT(des, fks_getDirEntNameStat_sub1, &cur, mt);
    if (cnt < 0) {
        //if (flags & FKS_DE_ErrCont)
        //  *ppAry    = buf;
        //else
