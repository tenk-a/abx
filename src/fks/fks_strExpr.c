 ...
 *
 *
 *  void fks_strExpr_setNameChkFunc(int (*name2valFnc)(char *name, int64_t *valp))
 *      式中に名前が現れたとき、その名前を値に変換するルーチンを登録する.
 *
 *      int name2valFnc(char *name, int64_t *valp)
 *
 *      のような関数をfks_strExpr利用者が作成し、fks_strExpr_setNameChkFuncで登録する.
...
/// 一文字取得.
static FKS_FORCE_INLINE void ch_get(void)
{
 ...
 */
FKS_LIB_DECL(int)
fks_strExpr(const char *s, const char **s_nxt, val_t *val)
{
    expr_err = 0;
    ch_p = (char *)s;
    *val = expr();
 ...
 *      int name2valFnc(char *name, int64_t *valp)<br>
 *  のような関数をfks_strExpr利用者が作成し、fks_strExpr_setNameChkFuncで登録する.
 *  利用者が作る関数は、nameを受け取り、値にするならば、その値を *valp
 *  にいれ、0を返す。値に出来ないならば 非0を返すこと.
 */
FKS_LIB_DECL(void)
fks_strExpr_setNameChkFunc(int (*name2valFnc)(char *name, val_t *valp))
{
