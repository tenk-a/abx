...
{
	printf(
		"ABX(w95dos/32)用 暫定版 v2.02                                            by tenk*\n"
	...
	for (f = i = 1; i < argc; i++) {
		...
		if (f == 0) {
		  LLL1:
			...
			++p;
			if (*p == '\\' || *p == '/' || p[1] == ':') {
				FIL_FullPath(Opt_abxName, p);
			} else {
				strcpy(CC_obuf, argv[0]);
				strcpy(FIL_BaseName(CC_obuf), p);
				FIL_FullPath(Opt_abxName, CC_obuf);
			}
